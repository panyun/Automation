	...
	public partial class IndexForm : Form
	{
		...
		public IndexForm()
		{
			...
			designComponent.RefreshNodeAction = (x, y) =>
			{
				...
				this.Invoke(() =>
				{
					AppendTextColorfulCmd(rtxt_msg, y, true);
				});
		...
		private void InitRichTextBoxContextMenu(RichTextBox textBox)
		{
			...
			{
				var node = GetSelectNode();
				var component = designComponent.GetComponentInfo(node.ComponentName);
				component.GetConfig();
				ClearCurrentComponent();
		...
		public void AppendTextColorfulCmd(RichTextBox rtBox, string addtext, bool IsaddNewLine, int loc = 0)
		{
			if (IsaddNewLine)
				addtext += Environment.NewLine;
			rtBox.SelectionStart = rtBox.TextLength;
			rtBox.SelectionLength = 0;
			rtBox.SelectionFont = new Font("黑体", 11, FontStyle.Underline);
			rtBox.SelectionColor = Color.Blue;
			rtBox.AppendText(addtext.Substring(0, 4));
			rtBox.SelectionStart = rtBox.TextLength;
			rtBox.SelectionLength = 0;
			rtBox.SelectionFont = new Font("黑体", 10);
			rtBox.SelectionColor = Color.Black;
			rtBox.AppendText(addtext.Substring(4, addtext.Length - 4));
			rtBox.SelectionColor = rtBox.ForeColor;
			rtBox.HideSelection = false;
		}
		...
		private void button2_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				if (insertNode != null)
				{
					var index = designComponent.CurrentDesignFlow.Steps.IndexOf(insertNode);
					if (index >1)
					{
