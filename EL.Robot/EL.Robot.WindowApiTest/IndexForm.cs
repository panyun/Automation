...
namespace EL.Robot.WindowApiTest
{
	public partial class IndexForm : Form
	{
		...
		private Node GetSelectNode()
		{
			...
			{
				string id;
				int index = rtxt_msg.GetFirstCharIndexOfCurrentLine();
				id = rtxt_msg.SelectedText;
				if (id.Length != 4)
					id = rtxt_msg.Text.Substring(index, 4);
				var nodes = designComponent.CurrentDesignFlow.Steps.Where(x => x.DisplayExp.Contains(id)).ToList();
				if (nodes == null || nodes.Count != 1)
				{
					WriteLog("请选中蓝色操作标识符！");
					return default;
				}
				if (nodes[0].ComponentName == nameof(StartComponent))
				{
					WriteLog("开始节点不能有多个！");
					return default;
				}
		...
		private void InitRichTextBoxContextMenu(RichTextBox textBox)
		{
			...
			rtxt_msg.DragDrop += (x, y) =>
			{
				...
				rtxt_msg.SelectionStart = charv;
				rtxt_msg.SelectionStart = rtxt_msg.GetFirstCharIndexOfCurrentLine();
				var id = rtxt_msg.Text.Substring(rtxt_msg.SelectionStart, 4);
				var node = designComponent.CurrentDesignFlow.Steps.Find(x => x.DisplayExp.Contains(id));
				var currentIndex = designComponent.CurrentDesignFlow.Steps.IndexOf(node);
				var id1 = y.Data.GetData(DataFormats.Text).ToString();
				var node1 = designComponent.CurrentDesignFlow.Steps.Find(x => x.DisplayExp.Contains(id1));
				if(node == null || node1 ==null || currentIndex> designComponent.CurrentDesignFlow.Steps.Count)
				{
					WriteLog("拖动失败！");
					return;
				}
				designComponent.CurrentDesignFlow.Steps.Remove(node1);
				node1.IsNew = true;
				designComponent.CurrentDesignFlow.Steps.Insert(currentIndex, node1);
			...
			update.Click += (x, y) =>
			{
				...
				//CreateParamDic(CurrentConfig.Parameters);
			...
			insert.Click += (x, y) =>
			{
				if (insert.Text == "当前插入")
				{
					...
					insert.Text = "取消当前";
					WriteLog($"已将[{insertNode.DisplayExp}]命令后插入新节点");
				}
				else
				{
					insert.Text = "当前插入";
		...
		private void button2_Click(object sender, EventArgs e)
		{
			...
			if (IsEdit && EditNode != null)
			{
				...
				end = EditNode.DisplayExp.Length;
				designComponent.RefreshAllStepCMD();
				IsEdit = false;
				EditNode = default;
