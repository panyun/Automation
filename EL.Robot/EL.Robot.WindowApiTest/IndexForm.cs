...
namespace EL.Robot.WindowApiTest
{
    public partial class IndexForm : Form
    {
        ...
        public IndexForm()
        {
            ...
            designComponent.RefreshNodeCmdEndAction = async () =>
            {
                ...
                if (start > 0 && end > 0)
                {
                    ...
                    rtxt_msg.SelectionColor = Color.DarkGreen;
                    rtxt_msg.SelectedText = SelectText;
                    rtxt_msg.SelectionStart = start;
        ...
        public void AppendTextColorfulCmd(string addtext, bool IsaddNewLine, Node node)
        {
            ...
            rtxt_msg.SelectionLength = 0;
            rtxt_msg.AppendText(addtext.Substring(0, num.Length));
            ...
            rtxt_msg.AppendText(addtext.Substring(num.Length, addtext.Length - num.Length));
            rtxt_msg.TabStop = true;
        ...
        private Node GetSelectNode()
        {
            try
            {
                ...
                var node = designComponent.CurrentDesignFlow.DesignSteps[index - 1];
        ...
        private void InitRichTextBoxContextMenu(RichTextBox textBox)
        {
            ...
            paste.Click += (x, y) =>
            {
                ...
                if (node == null) return;
                var index = designComponent.CurrentDesignFlow.DesignSteps.IndexOf(node) + 1;
            ...
            delete.Click += (x, y) =>
            {
                ...
                var row = designComponent.CurrentDesignFlow.DesignSteps.IndexOf(node) - 1;
                if (row < designComponent.CurrentDesignFlow.DesignSteps.Count)
                    end = designComponent.CurrentDesignFlow.DesignSteps[row].DisplayExp.Length;
                start = start - end - 1;
                designComponent.CurrentDesignFlow.DesignSteps.Remove(node);
                if (node.LinkNode != null)
                {
                    designComponent.CurrentDesignFlow.DesignSteps.Remove(node.LinkNode);
                }
        ...
        public async void RefreshRobots(long flowId, bool isVisible = true)
        {
            ...
            if (isVisible && flowId != default)
            {
                ...
                TestForm testForm = new TestForm(flow);
                testForm.Show();
                return;
                //加载流程
            }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var parameters = JsonHelper.FromJson<List<Parameter>>(JsonHelper.ToJson(ParamDic.Values.ToList()));
                //var index = rtxt_msg.SelectionStart;
                if (insertNode != null)
                {
                    var index = designComponent.CurrentDesignFlow.DesignSteps.IndexOf(insertNode) + 1;
