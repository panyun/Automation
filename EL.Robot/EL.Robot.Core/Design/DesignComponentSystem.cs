...
namespace EL.Robot.Core
{
    public static class DesignComponentSystem
    {
        ...
        public static void RefreshAllStepCMD(this DesignComponent self)
        {
            ...
            foreach (var step in self.CurrentDesignFlow.Steps)
            {
                self.RefreshStepCMD(step);
            }
        ...
        {
            var index = self.CurrentDesignFlow.Steps.IndexOf(node) + 1;
        ...
        public static bool CreateNode(this DesignComponent self, Node node)
        {
            ...
            self.CreateNode(node, self.CurrentDesignFlow.Steps.Count);
            return true;
        }
        public static bool CreateNode(this DesignComponent self, Node node, int index)
        {
            ...
            node.IsNew = true;
            self.FindParent(index);
            self.CurrentDesignFlow.Steps.Insert(index, node);
            if (node.ComponentName == nameof(IFStartComponent))
            {
                ...
                node.LinkNode = temp;
                self.CurrentDesignFlow.Steps.Insert(index + 1, temp);
            }
            self.RefreshAllStepCMD();
            var parameter = node.Parameters.FirstOrDefault(x => x.Key == nameof(Node.OutParameterName));
        ...
        {
            var link = self.CurrentDesignFlow.Steps.Where(x => x.LinkNode != null);
            Node node = default;
            for (int i = index; i > 0; i--)
            {
                node = self.CurrentDesignFlow.Steps[i];
                if (node.LinkNode != null)
                {
                    var endIndex = self.CurrentDesignFlow.Steps.IndexOf(node.LinkNode);
