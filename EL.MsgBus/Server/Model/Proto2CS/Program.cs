...
namespace ET
{
    ...
    public static class InnerProto2CS
    {
        ...
        public static void Proto2CS()
        {
            ...
            //GenerateOpcode("ET", "OuterOpcode", clientMessagePath + "CSharp/");
            //GenerateOpcodeC("ET", "protoCmd", clientMessagePath + "C/");
        }
        ...
        public static void Proto2CS(string ns, string protoName, string outputPath, string opcodeClassName, int startOpcode)
        {
            ...
            sb.Append("using Object = ET.Object;\n");
            ...
            GenerateOpcodeC(ns, opcodeClassName, outputPath);
        ...
        private static void GenerateOpcode(string ns, string outputFileName, string outputPath)
        {
            ...
            foreach (OpcodeInfo info in msgOpcode)
            {
                ...
                if (info.Opcode > maxOpcode)
                {
                    maxOpcode = info.Opcode;
                }
            }

            sb.AppendLine($"\n\t\t public const ushort MAX = {maxOpcode};");
            sb.AppendLine("\t}");
            sb.AppendLine("}");
        ...
        private static void GenerateOpcodeC(string ns, string outputFileName, string outputPath)
        {
            if (!Directory.Exists(outputPath))
            {
                Directory.CreateDirectory(outputPath);
            ...
            if (ns.Split(".").Length > 2)
                sb.AppendLine($"namespace ProtoCMD{ns.Split(".")[ns.Split(".").Length - 1]} {{");
            else
                sb.AppendLine("namespace Net {");
            sb.AppendLine("\tenum CMD");
            sb.AppendLine("\t {");
            foreach (OpcodeInfo info in msgOpcode)
            {
            ...
            sb.AppendLine("}");
            if (ns.Split(".").Length > 2)
                outputFileName = "ProtoCMD_" + ns.Split(".")[ns.Split(".").Length - 1];
            else
                outputFileName = "ProtoCMD";
            string csPath = Path.Combine(outputPath, outputFileName + ".h");
