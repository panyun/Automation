    ...
    public static class ExcelExporter
    {
        ...
        public static void Export()
        {
            try
            {
                ...
                var paths = Directory.GetFiles(excelDir);
        ...
        static void ExportSheetJson(ExcelWorksheet worksheet, ConfigType configType, StringBuilder sb)
        {
            ...
            for (int row = 6; row <= worksheet.Dimension.End.Row; ++row)
            {
                ...
                for (int col = 3; col <= worksheet.Dimension.End.Column; ++col)
                {
                    ...
                    {
                        Console.WriteLine($"Message : {ex.Message}; ConfigName:{worksheet.Name}; col:{col}; row:{row}");
                        Log.Debug($"Message : {ex.Message}; ConfigName:{worksheet.Name}; col:{col}; row:{row}");
                        throw;
                    }
        ...
        private static void ExportExcelProtobuf(ConfigType configType)
        {
            ...
            foreach (string protoName in protoNames)
            {
                ...
                {
                    Console.WriteLine($"Message : {ex.Message}; ConfigName:{protoName}; json:{json}  ");
                    Log.Debug($"Message : {ex.Message}; ConfigName:{protoName};  json:{json}");
                    throw;
                }
