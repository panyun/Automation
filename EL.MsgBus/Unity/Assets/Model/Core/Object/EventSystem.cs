...
namespace ET
{
    ...
    public sealed class EventSystem : IDisposable
    {
        ...
        {
            private readonly ConcurrentDictionary<Type, OneTypeSystems> typeSystemsMap = new ConcurrentDictionary<Type, OneTypeSystems>();
            ...
            public OneTypeSystems GetOrCreateOneTypeSystems(Type type)
            {
                ...
                this.typeSystemsMap.TryAdd(type, systems);
                return systems;
            }
        ...
        private readonly ConcurrentDictionary<long, Entity> allEntities = new ConcurrentDictionary<long, Entity>();
        ...
        private readonly ConcurrentDictionary<Type, List<object>> allEvents = new ConcurrentDictionary<Type, List<object>>();
        ...
        public void Add(Assembly assembly)
        {
            ...
            foreach (Type type in types[typeof(EventAttribute)])
            {
                ...
                {
                    this.allEvents.TryAdd(eventType, new List<object>());
                }
        ...
        public void RegisterSystem(Entity component, bool isRegister = true)
        {
            ...
            this.allEntities.TryAdd(component.InstanceId, component);
        ...
        {
            this.allEntities.TryRemove(instanceId,out _);
        }
