...
namespace EL.SignalingService.Model
{
    ...
    public class WebSocketServer
    {
        ...
        /// 异步发送队列
        /// </summary>
        private ConcurrentDictionary<UserToken, ConcurrentQueue<byte[]>> Sends = new ConcurrentDictionary<UserToken, ConcurrentQueue<byte[]>>();
        /// <summary>
        /// 开始监听
        /// </summary>
        /// <returns></returns>
        public void Listen()
        {
            ...
            catch (HttpListenerException e)
            {
                if (e.ErrorCode == 5)
                {
                    Log.Write($"CMD管理员权限 输入: netsh http add urlacl url=http://*:{ListenPort}/ user=Everyone");
                    ...
                    while (userToken.WebSocket != null && userToken.WebSocket.State == WebSocketState.Open)
                    {
                        if (sendQueues?.Count > 0)
                        {
                            while (true)
                            {
                                ...
                                {
                                    await userToken.WebSocket.SendAsync(new ArraySegment<byte>(data), WebSocketMessageType.Text, true, CancellationToken.None);
                                }
                                else
                                {
                                    break;
