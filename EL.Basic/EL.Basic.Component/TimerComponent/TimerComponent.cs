...
namespace EL.Async
{
    ...
    public class TimerComponent : Entity
    {
        ...
        private void Run(TimerAction timerAction)
        {
            switch (timerAction.TimerClass)
            {
                case TimerClass.OnceWaitTimer:
                    {
                        ELTask<bool> tcs = timerAction.Callback as ELTask<bool>;
        ...
        public async ELTask<bool> WaitTillAsync(long tillTime, ELCancellationToken cancellationToken = null)
        {
            ...
            ELTask<bool> tcs = ELTask<bool>.Create(true);
        ...
        public async ELTask<bool> WaitFrameAsync(ELCancellationToken cancellationToken = null)
        {
            return await WaitAsync(1, cancellationToken);
        }

        public async ELTask<bool> WaitAsync(long time, ELCancellationToken cancellationToken = null)
        {
            if (time == 0)
            {
                return true;
            ...
            timer.TimerClass = TimerClass.OnceWaitTimer;
            timer.Callback = tcs;
            timer.Time = 0;
            this.AddTimer(tillTime, timer);
            long timerId = timer.Id;
        ...
        /// 创建一个RepeatedTimer
        /// </summary>
        private long NewRepeatedTimerInner(long time, Action action, ELCancellationToken token = null)
        {
            long tillTime = TimeHelper.ServerNow() + time;
            TimerAction timer = this.AddChild<TimerAction>();
            timer.Time = time;
            timer.TimerClass = TimerClass.RepeatedTimer;
        ...
        public long NewOnceTimer(long tillTime, Action action)
        {
            ...
            timer.Time = 0;
            timer.TimerClass = TimerClass.OnceTimer;
            timer.Callback = action;
            this.AddTimer(tillTime, timer);
            return timer.Id;
        }
    ...
    public static class TimeHelper
    {
        ...
        {
            return Boot.TimeInfo.ClientNow();
        }
        ...
        {
            return Boot.TimeInfo.ServerNow();
        }

        public static DateTime ToDateTime(long timestamp)
        {
            return Boot.TimeInfo.ToDateTime(timestamp);
        }

        public static long ClientFrameTime()
        {
            return Boot.TimeInfo.ClientFrameTime();
        }

        public static long ServerFrameTime()
        {
            return Boot.TimeInfo.ServerFrameTime();
        }
    }
