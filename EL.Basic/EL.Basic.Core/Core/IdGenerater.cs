...
namespace EL
{
    ...
    public class IdGenerater : IDisposable
    {
        ...
        public IdGenerater()
        {
            ...
            {
                Log.Warn($"lastInstanceIdTime less than 0: {this.lastInstanceIdTime}");
                this.lastInstanceIdTime = 1;
            }
            this.lastIdTime = TimeSince2020();
            if (this.lastIdTime <= 0)
            {
                Log.Warn($"lastIdTime less than 0: {this.lastIdTime}");
                this.lastIdTime = 1;
            }
            this.lastUnitIdTime = TimeSince2020();
            if (this.lastUnitIdTime <= 0)
            {
                Log.Warn($"lastUnitIdTime less than 0: {this.lastUnitIdTime}");
        ...
        {
            uint a = (uint)((DateTime.Now.ToFileTimeUtc() - this.epoch2020) / 1000);
            return a;
        }

        private uint TimeSinceThisYear()
        {
            uint a = (uint)((DateTime.Now.ToFileTimeUtc() - this.epochThisYear) / 1000);
            return a;
        }

        public ulong GenerateInstanceId()
        {
